...
| 权限卡片闭环 | ✅ | ⚠️（待 Discord 组件化） | 后续可用 Button/Select 逐步补齐 |
...
- 飞书同款复杂权限卡片交互（Discord 当前仍以文本确认为主）
- 全量流式多卡分页渲染

后续会按 Discord 组件能力（Button/Select）分阶段补齐。
...
> **注意**: 本章节适用于 v2.8.2+ 版本，涉及路由器模式的灰度升级流程。如需详细操作指南，请参考 [`.sisyphus/evidence/task-16-fallback-recovery.txt`](.sisyphus/evidence/task-16-fallback-recovery.txt)。
